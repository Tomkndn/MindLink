...
module.exports.HandleTocreateAGroup = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: `Server error during group creation ${err} ` });
  }
};
...
module.exports.HandleToInvitePeople = async (req, res) => {
    ...
    if (existingParticipant) {
      if (existingParticipant.invited) {
        ...
        return res
          .status(400)
          .json({ message: "User is already part of the meeting" });
      }
    }
  ...
  try {
    ...
    if (meeting.isPublic) {
      if (!meeting.attendees.includes(userId)) {
        ...
        return res
          .status(200)
          .json({ message: "User joined the meeting successfully" });
      } else {
        console.log(meeting);
        return res
          .status(400)
          .json({ message: "User has already joined the meeting" });
      }
    } else {
      const existingParticipant = meeting.participants.find(
      ...
      if (existingParticipant && existingParticipant.invited) {
        ...
        return res
          .status(400)
          .json({ message: "User is not invited to the meeting" });
      }
    }
  } catch (error) {
...
module.exports.HandleToGetAllMeetings = async (req, res) => {
  ...
  try {
    ...
    if (!meetings || meetings.length === 0) {
      return res
        .status(404)
        .json({ message: "No meetings found created by this user" });
    }
...
module.exports.HandleToGetRecentMeetings = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "An error occurred while fetching recent meetings" });
  }
};
